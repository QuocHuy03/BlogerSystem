...
function getCookie(name) {
  console.log("huydev : ", document.cookie);
...
function sendAuthorizedRequest(url, method, data = null) {
  const accessToken =
  ...
  return $.ajax({
    ...
    if (
      jqXHR.status === 400 &&
      jqXHR.responseJSON &&
      jqXHR.responseJSON.message.includes("jwt expired")
    ) {
      // Gửi refreshToken lên server để làm mới accessToken
      refreshAccessToken()
        .then(function (data) {
          ...
          return sendAuthorizedRequest(url, method, data);
        })
